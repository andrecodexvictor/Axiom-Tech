...
graph TD
    ...
    Supervisor -->|Repo Docs/Code Structure| RepoAgent[Repo Agent]
    Supervisor -->|External Library/Doc Search| WebAgent[Web Research Agent]

    DocAgent --> Grader{Grade & Grounding Check}
    EngAgent --> Grader
    LegalAgent --> Grader
    RepoAgent --> Grader
    WebAgent --> Grader
...
| Agent Name | Scope & Responsibilities | Data Sources / Tools |
| :--- | :--- | :--- |
| **Supervisor Agent** | Classifies domain intent, routes queries, manages iteration state. | Intent Classifier Prompt + LangGraph Router |
| **Documentation Agent** | Responds to HR policies, onboarding, home office, internal comms. | Pinecone RAG (`rh`, `comms`) |
| **Engineering Agent** | Handles technical architecture, microservices map, SEV-1/2 incident guides. | Pinecone RAG (`engineering`, `ops`) |
| **Legal & Compliance Agent** | Answers LGPD privacy policy, internal terms of use, NDAs. | Pinecone RAG (`juridico`, `compliance`) |
| **Repo Agent** | Consults internal GitHub/GitLab repository structures and READMEs. | Internal Repo Parser Tool |
...
4. **Citation Engine**: Mandatory source attribution format: `[Source: document_name.pdf | Section/Page X]`.
